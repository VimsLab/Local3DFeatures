    ...
    if cfg.pretrained_path is not None:
        if cfg.mode == 'resume':
            ...
            if cfg.mode == 'test':
                # test mode
                # all_accs = []
                # for i in cfg.best_epochs:
                #     bepoch = "scanobjectnn-train-pointnext-s-ngpus1-seed6166-20221123-033644-HTGTG7oibFn8QTYMsuofTT_ckpt_latest_"+str(i)+".pth"
                # new_pretrained_path = os.path.join(cfg.pretrained_path, bepoch)
                epoch, best_val = load_checkpoint(
                    model, cfg.best_epochs, pretrained_path=cfg.pretrained_path)
                macc, oa, accs, cm = validate_fn(model, test_loader, cfg)
                    # all_accs.append(np.array(accs))
                #     print(accs)
                # print(np.max(np.stack(all_accs),axis=0))
                # print(np.mean(np.max(np.stack(all_accs),axis=0)))
                # print(np.array(cfg.per_class))
                # print(np.max(np.stack(all_accs),axis=0)*np.array(cfg.per_class))
                # print(np.sum(np.max(np.stack(all_accs),axis=0)*np.array(cfg.per_class))/2882)
                # exit()
