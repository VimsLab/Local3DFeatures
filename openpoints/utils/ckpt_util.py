...
def save_checkpoint(cfg, model, epoch, optimizer=None, scheduler=None,
    ...
    current_ckpt_name = f'{save_name}_{post_fix}_{epoch}.pth'
...
def load_checkpoint(model, best_epochs, pretrained_path, module=None):

    # load state dict
    state_dict = []
    for i in range(len(best_epochs)):
        bepoch = str(best_epochs[i])+".pth"
        new_pretrained_path = os.path.join(pretrained_path, bepoch)
        print(new_pretrained_path)
        if not os.path.exists(pretrained_path):
            raise NotImplementedError('no checkpoint file from path %s...' % new_pretrained_path)
        state_dict.append(torch.load(new_pretrained_path, map_location='cpu'))

    new_state_dict = state_dict[0]#['model'].values()
    # print(new_state_dict['model'].keys())
    # exit()
    temp = 0
    for i in range(len(state_dict)-1):
        for j in state_dict[0]['model'].keys():
            # if 'convs' in j:
            #     state_dict[0]['model'][j] = state_dict[0]['model'][j] + state_dict[i+1]['model'][j]
            # else:
                state_dict[0]['model'][j] = state_dict[0]['model'][j]
            # print(state_dict[i+1]['model'][j])
            # exit()
    #         new_state_dict[0]['model'][j] += state_dict[i+1]['model'][j]
    # for j in state_dict[0]['model'].keys():
    #     if 'convs' in j:
    #         state_dict[0]['model'][j] = state_dict[0]['model'][j]/len(best_epochs)
