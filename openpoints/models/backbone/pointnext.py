...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        channels[0] = in_channels if is_head else CHANNEL_MAP[feature_type](channels[0])+4
    ...
    def forward(self, pf):
    # def forward(self, pnf):
        p, f = pf
        # p, new_n, f = pnf
        # print(p.shape,n.shape,f.shape)
        if self.is_head:
            f = self.convs(f)  # (n, c)
        else:
            ...
            # dp, fj = self.grouper(new_p, p, new_n, f)
...
class PointNextEncoder(nn.Module):
    ...
    def forward_cls_feat(self, p0, f0=None):
        ...
        # n0 = estimate_pointcloud_normals(p0,32)
        for i in range(0, len(self.encoder)):
            # p0, n0, f0 = self.encoder[i]([p0, n0, f0])
