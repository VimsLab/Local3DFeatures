...
class BallQuery(Function):
    ...
    def forward(ctx, radius: float, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor) -> torch.Tensor:
        ...
        distance = torch.cuda.FloatTensor(B, npoint, nsample, device=xyz.device).zero_()
        pointnet2_cuda.ball_query_wrapper(B, N, npoint, radius, nsample, new_xyz, xyz, idx, distance)
        return idx, distance
...
class QueryAndGroup(nn.Module):
    def __init__(self, radius: float, nsample: int,
        ...
        """
        super().__init__()
        self.radius, self.nsample = radius, int(nsample)
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[
        torch.Tensor]:
    # def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, normals: torch.Tensor, features: torch.Tensor = None) -> Tuple[
    #     torch.Tensor]:
        """
        ...
        """
        idx, distance = ball_query(self.radius, self.nsample, support_xyz, query_xyz)

        if self.return_only_idx:
            return idx
        xyz_trans = support_xyz.transpose(1, 2).contiguous()
        grouped_xyz = grouping_operation(xyz_trans, idx)#.transpose(1,2)).transpose(2,3)  # (B, 3, npoint, nsample)
        ...
        grouped_features = torch.cat((grouped_xyz, grouped_features, torch.unsqueeze(distance, dim=1)),dim=1)
...
class GroupAll(nn.Module):
    ...
    # def forward(self, new_xyz: torch.Tensor, xyz: torch.Tensor, normals: torch.Tensor, features: torch.Tensor = None):
        """
        :param xyz: (B, N, 3) xyz coordinates of the features
